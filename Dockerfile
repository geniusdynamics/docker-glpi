...
RUN echo "#!/bin/bash" > /usr/local/bin/db_setup.sh && \
    echo "set -e" >> /usr/local/bin/db_setup.sh && \
    echo "" >> /usr/local/bin/db_setup.sh && \
    echo "if [[ -f /docker-entrypoint-initdb.d/zz_glpi_restore.sh ]]; then" >> /usr/local/bin/db_setup.sh && \
    echo "    echo \"Database restore script found (likely from Podman setup). Skipping restore logic.\"" >> /usr/local/bin/db_setup.sh && \
    echo "else" >> /usr/local/bin/db_setup.sh && \
    echo "    if mysql --host=\"\$MARIADB_DB_HOST\" --user=\"\$MARIADB_DB_USER\" --password=\"\$MARIADB_DB_PASSWORD\" --execute=\"SHOW DATABASES LIKE '\$MARIADB_DB_NAME';\" | grep -q \"\$MARIADB_DB_NAME\" ; then" >> /usr/local/bin/db_setup.sh && \
    echo "        echo \"Database backup found on external database. Restoring...\"" >> /usr/local/bin/db_setup.sh && \
    echo "        mysqldump --host=\"\$MARIADB_DB_HOST\" --user=\"\$MARIADB_DB_USER\" --password=\"\$MARIADB_DB_PASSWORD\" \"\$MARIADB_DB_NAME\" | mysql --host=localhost --user=\"\$MARIADB_DB_USER\" --password=\"\$MARIADB_DB_PASSWORD\" \"\$MARIADB_DB_NAME\"" >> /usr/local/bin/db_setup.sh && \
    echo "    else" >> /usr/local/bin/db_setup.sh && \
    echo "        echo \"No database backup found. Performing clean install...\"" >> /usr/local/bin/db_setup.sh && \
    echo "        /usr/bin/php /var/www/html/glpi/bin/console glpi:database:install \\" >> /usr/local/bin/db_setup.sh && \
    echo "              --reconfigure \\" >> /usr/local/bin/db_setup.sh && \
    echo "              --no-interaction \\" >> /usr/local/bin/db_setup.sh && \
    echo "              --force \\" >> /usr/local/bin/db_setup.sh && \
    echo "              --db-host=\"\$MARIADB_DB_HOST\" \\" >> /usr/local/bin/db_setup.sh && \
    echo "              --db-port=\"\$MARIADB_DB_PORT\" \\" >> /usr/local/bin/db_setup.sh && \
    echo "              --db-name=\"\$MARIADB_DB_NAME\" \\" >> /usr/local/bin/db_setup.sh && \
    echo "              --db-user=\"\$MARIADB_DB_USER\" \\" >> /usr/local/bin/db_setup.sh && \
    echo "              --db-password=\"\$MARIADB_DB_PASSWORD\"" >> /usr/local/bin/db_setup.sh && \
    echo "    fi" >> /usr/local/bin/db_setup.sh && \
    echo "fi" >> /usr/local/bin/db_setup.sh && \
